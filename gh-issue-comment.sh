...
cat "${1}" > issue-commentmd

if test -n "${ARTIFACT_IDS}";then
	sed -e 's/^	//'>>issue-comment.md<<END_OF_COMMENT
...
if test -z "${comment}"; then
	"${DRY_RUN}" || gh api "repos/{owner}/{repo}/issues/${ISSUE_ID}/comments" --field 'body=@issue-comment.md'
else
	comment_id="$(printf '%s' "${comment}" | jq '.id')"
	"${DRY_RUN}" || gh api --method PATCH "repos/{owner}/{repo}/issues/comments/${comment_id}" --field 'body=@issue-comment.md'
fi
