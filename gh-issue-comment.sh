...
_gh_auth_id()
{
	gh auth status 2>&1 | sed -E -n 's/.*Logged in to [^[:space:]]+ as ([^[:space:]]+) \(.*\)/\1/p'
}
gh_user() { gh api "users/$(_gh_auth_id)" --jq '.login'; }
gh_uid() { gh api "users/$(_gh_auth_id)" --jq '.id'; }
...
cleanup() { rm -f '/tmp/gh-issue-comment.md'; }
trap cleanup 0 EXIT
cat "${1}" > '/tmp/gh-issue-comment.md'

if test -n "${ARTIFACT_IDS}";then
	sed -e 's/^	//'>>'gh-issue-comment.md'<<END_OF_COMMENT
...
if test -z "${comment}"; then
	"${DRY_RUN}" || gh api "repos/{owner}/{repo}/issues/${ISSUE_ID}/comments" --field 'body=@/tmp/gh-issue-comment.md'
else
	comment_id="$(printf '%s' "${comment}" | jq '.id')"
	"${DRY_RUN}" || gh api --method PATCH "repos/{owner}/{repo}/issues/comments/${comment_id}" --field 'body=@/tmp/gh-issue-comment.md'
fi
